        ...
        function parseStartTag() {
            ...
            if (start) {
                ...
                while (
                    ...
                    //如果不是 > 标签  并且是attribute 比如<div id=app> 先得到app 第二次while 的dao>
                    advance(attr[0].length); // 属性加入
                    match.attrs.push(attr);
                }
                if (end) {
                    //  第一while匹配到 attr 第二次就能end到                在第二次while循环后 end匹配到结束标签 => ['>','']
    ...
    function Parse(template) {
        ...
        parseHTML(template, {
            start: function start(tag, attrs, unary) {
                ...
                var element =
                    stack[stack.length - 1]; /*从stack中取出最后一个ele*/
                var lastNode =
                    element.children[
